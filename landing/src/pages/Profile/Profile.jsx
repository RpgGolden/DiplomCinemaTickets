...
function Profile() {
  ...
  <div
    style={{
      display: 'inline-flex',
      alignItems: 'center',
      justifyContent: ticket.ticketStatus === 'purchased' ? 'space-between' : 'center',
      gap: '8px',
    }}
  >
    <div className={`${styles.status} ${statusClassMap[ticket.ticketStatus] || ''}`}>
      <span>
        {ticket.ticketStatus ? getTicketStatusText(ticket.ticketStatus) : '-'}
      </span>
    </div>
    ...
    {ticket.ticketStatus === 'purchased' && (
      <button
        ...
        onClick={() => handeConfirmCancelTicket(ticket.ticketId)}
      >
        <svg
